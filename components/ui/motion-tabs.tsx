...
function TabsContents({
  ...
  return (
    <div data-slot='tabs-contents' className={cn('overflow-hidden', className)} {...props}>
      <motion.div className='-mx-2 flex h-full min-h-0' animate={{ x: activeIndex * -100 + '%' }} transition={transition}>
        {childrenArray.map((child, index) => (
          <div key={index} className='w-full shrink-0 px-2 h-full min-h-0'>
            {child}
          </div>
        ))}
      </motion.div>
