...
function MotionHighlight<T extends string>({ ref, ...props }: MotionHighlightProps<T>) {
  ...
  const safeSetBoundsRef = React.useRef(safeSetBounds)
  React.useEffect(() => {
...
function MotionHighlightItem({
  ...
  const setRef = (instance: HTMLDivElement | null) => {
    (localRef as React.MutableRefObject<HTMLDivElement | null>).current = instance
    const childRef = (element as React.ReactElement & { ref?: React.Ref<HTMLDivElement> }).ref
    if (typeof childRef === 'function') {
      childRef(instance)
    } else if (childRef && typeof childRef === 'object') {
      queueMicrotask(() => {
        (childRef as React.MutableRefObject<HTMLDivElement | null>).current = instance
      })
    }
  }
